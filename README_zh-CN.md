...
- 🔒 **云端规划，本地执行**  
  AI Agent 负责理解用户需求、规划任务流程，本地运行时负责访问文件、邮件、浏览器及企业系统等敏感资源，实现数据不出本地的安全执行模式。

- 🧩 **基于 Manifest 的技能发现机制**  
  所有技能均通过结构化 Manifest 描述能力、参数及约束，Agent 可按需发现技能并获取详细定义，无需一次加载全部工具信息。

- 🔄 **灵活的工作流编排能力**  
  同时支持数据库中保存的可复用工作流、AI 临时生成的多步骤工作流，以及单个内置技能的直接执行，满足不同自动化需求。

- 📁 **丰富的本地与企业系统集成能力**  
  提供文件管理、邮件处理、浏览器自动化、截图、桌面程序调用、企业微信等多种内置能力，并支持扩展更多本地或企业系统。

- 🌐 **统一的 REST / OpenAPI 接口**  
  提供统一的能力发现与执行接口，可方便接入 Coze、Dify、GPTs、自定义 Agent、桌面客户端等不同 AI 平台。

- 🛡️ **以安全为核心的执行机制**  
  支持路径白名单、程序白名单、策略校验、执行日志、权限控制等安全措施，降低 AI 自动执行带来的潜在风险。
