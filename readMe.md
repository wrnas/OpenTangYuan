...
1. 为智能体创建插件，建议创建4个插件，分别对应本项目的四个主要方法：
| 插件名称 | 方法名路径 |描述
|---|---|---|
| GetSkillListForAI | Skills/GetSkillListForAI |获取系统当前可用的技能总览。优先用于让智能体判断“是否已有可直接复用的 workflow”。 返回两类信息： 1. workflows：数据库中已保存的工作流技能，优先直接调用； 2. builtins：系统内置原子技能，如 browser_task、file_task、tool_task、email_task、wechat_task。 使用规则： 1. 用户提出任务后，优先先调用本工具； 2. 如果返回中已有合适 workflow，则继续调用 GetSkillAction 查看详情，再调用 ExecuteSkill； 3. 如果没有合适 workflow，再考虑调用 ExecuteSkill 组合 Steps，或调用 AiBrowser 处理底层浏览器动作|
| GetSkillAction | Skills/GetSkillListForAI |根据 SkillCode 获取某个已保存 workflow 的详细定义。 适用场景： 1. 已经通过 GetSkillListForAI 找到疑似可用的 workflow； 2. 需要进一步查看该 workflow 的用途、步骤、参数； 3. 在正式执行前确认是否适合当前任务。 输入规则： - 只传 SkillCode； - SkillCode 必须来自 GetSkillListForAI 返回的 workflows。 输出内容通常包括： - skillCode - remark - skillType - updateTime - steps - skillActionsRaw 使用建议： - 不要跳过本工具直接盲目执行 workflow； - 先读详情，再决定是否调用 ExecuteSkill。|
| ExecuteSkill | Skills/ExecuteSkillForCoze |统一执行入口，用于执行 builtin 技能、临时 workflow、已保存 workflow。通过json字符串的形式来传参。|
| GetBuiltinSkillDetail | Skills/GetBuiltinSkillDetail |用来获取BuiltinSkill的详情，包括用法，参数等。|

2. 为智能体设置提示词（以Coze智能体为例）。
提示词较长，请在如下位置查看。
