...
建议创建 4 个插件，分别对应 OpenTangYuan 的四个核心方法：

| 插件名称 | 方法名路径 | 描述 |
|---|---|---|
| GetSkillListForAI | `Skills/GetSkillListForAI` | 获取系统当前可用的技能总览。优先用于让智能体判断“是否已有可直接复用的 workflow”。返回两类信息：(1) workflows：数据库中已保存的工作流技能，优先直接调用；(2) builtins：系统内置原子技能，如 browser_task、file_task、tool_task、email_task、wechat_task。使用规则：(a) 用户提出任务后，优先调用本工具；(b) 如果返回中已有合适 workflow，则继续调用 GetSkillAction 查看详情，再调用 ExecuteSkill；(c) 如果没有合适 workflow，则考虑调用 ExecuteSkill 组合 Steps，或调用 AiBrowser 处理底层浏览器动作。 |
| GetSkillAction | `Skills/GetSkillAction` | 根据 SkillCode 获取某个已保存 workflow 的详细定义。适用场景：(a) 已通过 GetSkillListForAI 找到疑似可用的 workflow；(b) 需要进一步查看该 workflow 的用途、步骤和参数；(c) 在正式执行前确认是否适合当前任务。输入规则：只传 SkillCode，且必须来自 GetSkillListForAI 返回的 workflows。输出内容通常包括：skillCode、remark、skillType、updateTime、steps、skillActionsRaw。使用建议：先读详情，再决定是否调用 ExecuteSkill，不要跳过本工具直接盲目执行。 |
| ExecuteSkill | `Skills/ExecuteSkillForCoze` | 统一执行入口，用于执行 builtin 技能、临时 workflow 或已保存 workflow。参数通过 JSON 字符串形式传递。 |
| GetBuiltinSkillDetail | `Skills/GetBuiltinSkillDetail` | 获取 BuiltinSkill 的详情，包括用法、参数和示例。 |

### 2. 为智能体设置提示词

以 Coze 智能体为例，提示词较长，请查看：
...
- 屏蔽 Swagger，或为 Swagger 设置用户认证（在 `appsettings` 中留有配置选项）。
- 在 `program.cs` 中找到 **启用 Swagger 用户验证** 部分，取消代码注释即可。
- 开启 ApiKey 验证，即在控制器中增加 `[Authorize(AuthenticationSchemes = "ApiKey")]` 选项，举例如下：
